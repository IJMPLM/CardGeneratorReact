...
import { CardConfig, OverlayType, BorderColor, BorderGradients, defaultCardConfig } from '../types/CardTypes';
import '../styles/CardGenerator.css';

// Font loading - still need to access fonts from public directory
const fontFaces = `
  @font-face {
...
const CardGenerator: React.FC = () => {
  ...
  const [configText, setConfigText] = useState<string>(JSON.stringify(defaultCardConfig, null, 2));
  const [currentOverlay, setCurrentOverlay] = useState<OverlayType>('holographic');
  const [currentBorderColor, setCurrentBorderColor] = useState<BorderColor>('GREEN');
  const [cardElementsLoaded, setCardElementsLoaded] = useState<boolean>(false);
  ...
  const [previousBorderGradient, setPreviousBorderGradient] = useState<Array<number | string> | null>(null);
  ...
  const applyHoloEffect = () => {
    ...
    overlayAnimationRef.current = new window.Konva.Animation((frame: any) => {
      angle += 0.01;
    ...
    if (currentOverlay === 'foil' && effectType !== 'foil') {
      ...
      if (previousBorderGradient) {
        ...
        for (const option in borderGradients) {
          if (JSON.stringify(borderGradients[option]) === JSON.stringify(previousBorderGradient)) {
  ...
  const configureCard = (config: Partial<CardConfig>) => {    
    ...
    for (const key in config) {
      if (key in mergedConfig && config[key as keyof CardConfig] !== undefined) {
    ...
    if (critRateRef.current) {
      critRateRef.current.text('Crit Rate: ' + mergedConfig.critRateValue);
    }
  ...
  const exportAsPng = () => {
    if (stageRef.current) {
      ...
      link.download = `card_${cardConfig.cardTitle.replace(/\s+/g, '_').toLowerCase()}.png`;
      link.href = dataURL;
      link.click();
    }
  };
  ...
  const clipFunc = (ctx: any) => {
    ...
    ctx.quadraticCurveTo(dimensions.clipGroupWidth, 0, dimensions.clipGroupWidth, cornerRadius);
    ctx.lineTo(dimensions.clipGroupWidth, dimensions.clipGroupHeight - cornerRadius);
    ctx.quadraticCurveTo(dimensions.clipGroupWidth, dimensions.clipGroupHeight, dimensions.clipGroupWidth - cornerRadius, dimensions.clipGroupHeight);
    ctx.lineTo(cornerRadius, dimensions.clipGroupHeight);
    ctx.quadraticCurveTo(0, dimensions.clipGroupHeight, 0, dimensions.clipGroupHeight - cornerRadius);
    ...
    <div className="card-generator">
        ...
        >
          <Layer>
            ...
            <Rect
              ...
              fillLinearGradientStartPoint={{ x: 0, y: 0 }}
              fillLinearGradientEndPoint={{ x: cardWidth, y: cardHeight }}
              fillLinearGradientColorStops={borderGradients[currentBorderColor]}
            />
            ...
            >
              {cardElementsLoaded && (
                <Image
                  ref={imageRef}
                  image={imageRef.current?.image() || new window.Image()}
                  width={dimensions.clipGroupWidth}
                  height={dimensions.clipGroupHeight}
                  y={0}
                />
            ...
            {/* Crit Rate Box */}
            <Rect
              x={cardWidth - (18 * scaleFactor) - (cardWidth - (40 * scaleFactor)) / 2}
              y={cardHeight - (75 * scaleFactor)}
              width={(cardWidth - (40 * scaleFactor)) / 2}
              height={15 * scaleFactor}
              fill="white"
            ...
            <Text
              ref={critRateRef}
              x={cardWidth - (2 * scaleFactor) - (cardWidth - (40 * scaleFactor)) / 2}
              y={cardHeight - (75 * scaleFactor) + (2 * scaleFactor)}
      ...
      <div className="controls">
        <div className="control-row">
          ...
          <label htmlFor="border-color" style={{ marginLeft: '15px' }}>Border Color: </label>
          <select
            id="border-color"
        ...
        <button className="export-button" onClick={exportAsPng}>Export as PNG</button>
      </div>
    </div>
  );
};
